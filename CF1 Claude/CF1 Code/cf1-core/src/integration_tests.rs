...
mod integration_tests {
    ...
    use crate::state::{AssetDetails, ComplianceInfo, Document, FinancialTerms, ProposalStatus};
        ...
        AppBuilder::new().build(|router, _, storage| {
            ...
            router
                ...
                .init_balance(
                    ...
                    coins(10_000_000_000_000, "untrn"),
                )
                .unwrap();
            router
                .bank
    ...
    fn test_complete_investment_lifecycle() {
        ...
        assert!(create_res.events.iter().any(|e| e
            .attributes
            .iter()
            .any(|a| a.key == "proposal_id" && a.value == "proposal_1")));
        ...
        for (investor, amount) in investments {
            ...
            assert!(invest_res.events.iter().any(|e| e
                .attributes
                .iter()
                .any(|a| a.key == "method" && a.value == "invest")));
        }
        ...
        assert_eq!(
            proposal_res.proposal.funding_status.raised_amount,
            Uint128::from(4_500_000_000_000u128)
        );
        ...
        assert!(final_invest_res.events.iter().any(|e| e
            .attributes
            .iter()
            .any(|a| a.key == "funding_completed" && a.value == "true")));
        ...
        assert_eq!(
            final_proposal_res.proposal.funding_status.raised_amount,
        ...
        assert_eq!(
            user_investments.total_invested,
            Uint128::from(1_500_000_000_000u128)
        ); // $1000 + $500
    ...
    fn test_proposal_failure_and_refunds() {
        ...
        assert!(refund_res.events.iter().any(|e| e
            .attributes
            .iter()
            .any(|a| a.key == "method" && a.value == "refund_investors")));
        assert!(refund_res.events.iter().any(|e| e
            .attributes
            .iter()
            .any(|a| a.key == "total_refunded" && a.value == "1500000000000")));
    ...
    fn test_proposal_updates_and_permissions() {
        ...
        updated_asset_details
            .highlights
            .push("New HVAC system".to_string());
        ...
        assert!(update_res.events.iter().any(|e| e
            .attributes
            .iter()
            .any(|a| a.key == "method" && a.value == "update_proposal")));
        ...
        assert_eq!(
            updated_proposal.proposal.asset_details.description,
            "Updated premium office building"
        );
        ...
        assert!(cancel_res.events.iter().any(|e| e
            .attributes
            .iter()
            .any(|a| a.key == "method" && a.value == "cancel_proposal")));
        ...
        assert_eq!(
            cancelled_proposal.proposal.status,
    ...
    fn test_creator_stats_and_portfolio() {
        ...
        for i in 0..3 {
            let (mut asset_details, financial_terms, documents, compliance) =
        ...
        assert_eq!(
            creator_res.creator.total_raised,
            Uint128::from(5_000_000_000_000u128)
        );
        ...
        let funded_count = creator_proposals
            .proposals
            .iter()
            .filter(|p| p.proposal.status == ProposalStatus::Funded)
            .count();
        let active_count = creator_proposals
            .proposals
            .iter()
    ...
    fn test_investment_edge_cases() {
        ...
        assert!(below_min_err
            .to_string()
            .contains("Investment amount below minimum"));
        ...
        assert_eq!(
            investment_res.investment.amount,
