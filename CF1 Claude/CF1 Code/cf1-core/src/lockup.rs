...
use crate::state::{Proposal, ProposalStatus, PROPOSALS, TOKEN_CONTRACTS};
...
pub fn is_in_lockup_period(deps: Deps, env: &Env, proposal_id: &str) -> StdResult<bool> {
...
pub fn get_lockup_info(deps: Deps, env: &Env, proposal_id: &str) -> StdResult<LockupInfo> {
    let proposal = PROPOSALS.load(deps.storage, proposal_id.to_string())?;
    let token_address = TOKEN_CONTRACTS.load(deps.storage, proposal_id.to_string())?;
...
pub fn get_remaining_lockup_time(deps: Deps, env: &Env, proposal_id: &str) -> StdResult<u64> {
    let proposal = PROPOSALS.load(deps.storage, proposal_id.to_string())?;
...
pub fn process_expired_lockups(
    ...
    for (proposal_id, mut proposal) in proposals_to_process {
        ...
        unlocked_proposals.push(proposal_id);
        processed_count += 1;
    }
...
pub fn get_lockup_progress(deps: Deps, env: &Env, proposal_id: &str) -> StdResult<u64> {
    let proposal = PROPOSALS.load(deps.storage, proposal_id.to_string())?;
...
pub fn get_lockup_status(deps: Deps, env: &Env, proposal_id: &str) -> StdResult<LockupStatus> {
